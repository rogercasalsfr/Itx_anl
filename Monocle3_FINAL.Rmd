...
rownames(taula) <- c("Naive","Tfh","Th17","Treg")
taula
...
DoHeatmap(CD4totssubsetpre, features = top15$gene) + NoLegend()


post.markers %>%
    group_by(cluster) %>%
    top_n(n = 15, wt = avg_log2FC) -> top152
DoHeatmap(CD4totssubsetpost, features = top152$gene) + NoLegend()
...
totsCD4subsetpost <- order_cells(totsCD4subsetpost, reduction_method = 'UMAP', root_cells = colnames(totsCD4subsetpost[,clusters(totsCD4subsetpost) == "post_Naive"]))
...
Com a conclusions, obtenim que ara sí que podem observar bé la diferenciació de les cèl·lules de les "naive" . Hem de veure si les Treg que quedin allà significa alguna cosa important o que? o està calculant la trajectòria malament potser.
...
dittoHeatmap(CD4totssubsetpost, top15$gene, annot.by = c("celltype.cnd", "monocle3_pseudotime"), order.by= "celltype.cnd", scaled.to.max = FALSE, show_colnames = FALSE)
...
dittoHeatmap(CD4totssubsetpost, prova2, annot.by = c("celltype.cnd", "monocle3_pseudotime"), order.by=c( "monocle3_pseudotime", "celltype.cnd"), scaled.to.max = FALSE, show_colnames = FALSE)
...
best <- head(cds_pt_res[order(-cds_pt_res$morans_test_statistic),], n=1050)
best2 <- head(cds_pt_res2[order(-cds_pt_res2$morans_test_statistic),], n=1050)
...
dotplot(test_enrich,label_format = 100 ,showCategory=20, font.size=8) | dotplot(test_enrich2, label_format=100, showCategory=20, font.size=8)
...
#ens <- mapIds(EnsDb.Hsapiens.v79, keys=rownames(best), column="GENENAME", keytype = "ENTREZID")




#head((CD4totssubsetpre@assays$RNA@counts), n=5)

heattt <- head(cds_pt_res[order(-cds_pt_res$morans_test_statistic),], n=40)
liada <- subset(CD4totssubsetpre, features = rownames(heattt))
pre <- DoHeatmap(liada, group.by="celltype.cnd")



#universe= rownames(CD4totssubsetpre@assays$RNA@counts)
#head(gene_names, n = 5)
#best_genes <- gene_names[gene_names %in% best]



best <- head(cds_pt_res[order(-cds_pt_res$morans_test_statistic),], n=50)

genelist <- rownames(cds_pt_res)


best2 <- head(cds_pt_res2[order(-cds_pt_res2$morans_test_statistic),], n=50)

genelist2 <- rownames(cds_pt_res2)
...
GO_resultsprova <- enrichGO(gene=genelist, OrgDb = "org.Hs.eg.db", keyType = "SYMBOL", ont="BP", pvalueCutoff = 0.0000001, pAdjustMethod = "BH", qvalueCutoff = 0.0001, minGSSize = 10, maxGSSize = 500, readable=FALSE)





GO_resultsMF <- enrichGO(gene=genelist, OrgDb = "org.Hs.eg.db", keyType = "SYMBOL", ont="MF")
GO_resultsCC <- enrichGO(gene=genelist, OrgDb = "org.Hs.eg.db", keyType = "SYMBOL", ont="CC")

fit <-plot(barplot(GO_resultsprova, showCategory = 15))
fitMF <-plot(barplot(GO_resultsMF, showCategory = 15))
fitCC <-plot(barplot(GO_resultsCC, showCategory = 15))





GO_results2 <- enrichGO(gene=genelist2, OrgDb = "org.Hs.eg.db", keyType = "SYMBOL", ont="BP")
GO_resultsMF2 <- enrichGO(gene=genelist2, OrgDb = "org.Hs.eg.db", keyType = "SYMBOL", ont="MF")
GO_resultsCC2 <- enrichGO(gene=genelist2, OrgDb = "org.Hs.eg.db", keyType = "SYMBOL", ont="CC")

fit2 <-plot(barplot(GO_results2, showCategory = 15))
fitMF2 <-plot(barplot(GO_resultsMF2, showCategory = 15))
fitCC2 <-plot(barplot(GO_resultsCC2, showCategory = 15))




A <- GO_resultsMF@result[["Description"]] 
B <- GO_resultsMF2@result[["Description"]]
...
C <- GO_resultsCC@result[["Description"]] 
D <- GO_resultsCC2@result[["Description"]]
...
E <- GO_resultsprova@result[["Description"]] 
G <- GO_results2@result[["Description"]]
...
set.seed(1)
A <- GO_resultsprova@result[["Description"]] 
B <- GO_results2@result[["Description"]]
xtab_set(A,B)
...
x2 <- pairwise_termsim(GO_resultsprova)
x3 <- pairwise_termsim(GO_results2)


emapplot(x2, showCategory=15)
emapplot(x3, showCategory=15)
