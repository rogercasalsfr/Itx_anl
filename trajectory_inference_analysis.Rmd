...
cds_object <- order_cells(cds_object, reduction_method = 'UMAP', root_cells = colnames(cds_object[,clusters(cds_object) == "post_CD8_mem"]))  # We set up the root as naive cells.
...
markers <- FindMarkers(subset_seurat_obj, ident.1=c("post_CD8_act"), ident.2=c("post_CD8_ex"))  # Find DE genes based on Wilcoxon test based on the different conditions.
...
write.csv(markers, file = "/home/roger/data_immunotherapy/DEgenes/branch10.csv", row.names = FALSE)
